var CustomImportScript = (() => {
  ...
  function transform(hookName, element, payload) {
    ...
    if (hookName === "buttonize") {
      const { document } = payload;
      const buttons = element.querySelectorAll("a.sb-button");
      console.log(`[buttonize] Found ${buttons.length} sb-button links`);
      buttons.forEach((a) => {
        console.log(`[buttonize] Processing: ${a.textContent.trim()} href=${a.href}`);
        if (!a.closest("em") && !a.closest("strong")) {
          const em = document.createElement("em");
          a.parentNode.insertBefore(em, a);
          em.appendChild(a);
          console.log(`[buttonize] Wrapped in <em>: ${a.textContent.trim()}`);
        }
      });
    }
  ...
  var import_about_us_default = {
    transform: (payload) => {
      ...
      executeTransformers("buttonize", main, payload);
